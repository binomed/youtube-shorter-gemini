...
describe('StemService', () => {
    ...
    describe('separateStems', () => {
        it('should throw NotFoundException when project does not exist', async () => {
            ...
            await expect(service.separateStems('no-proj', 'short-1')).rejects.toThrow(NotFoundException);
        });
        ...
        it('should throw NotFoundException when short does not belong to project', async () => {
            ...
            await expect(service.separateStems('proj-1', 'no-short')).rejects.toThrow(NotFoundException);
        });
        ...
        it('should create a Job record (SQL-Queue) before starting', async () => {
            ...
            await expect(service.separateStems('proj-1', 'short-1')).rejects.toThrow('mkdir failed');

            // Job was created before the error
            expect(mockJobService.create).toHaveBeenCalledWith(
                expect.objectContaining({ type: 'stem_separation', projectId: 'proj-1', shortId: 'short-1' })
            );
        });
        ...
        it('should mark job as failed if separation throws (mkdir fails)', async () => {
            ...
            fs.mkdir.mockResolvedValue(undefined); // mkdir succeeds
            ...
            await expect(service.separateStems('proj-1', 'short-1')).rejects.toThrow('disk full');

            expect(mockJobService.fail).toHaveBeenCalledWith(mockJob.id, expect.stringContaining('disk full'));
        });
