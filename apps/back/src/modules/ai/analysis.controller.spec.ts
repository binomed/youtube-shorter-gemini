...
describe('AnalysisController', () => {
    ...
    describe('POST /projects/:projectId/analyze', () => {
        it('should trigger analysis and return 202 Accepted', async () => {
            ...
            const result = await controller.startAnalysis?.('proj-1');

            if (result !== undefined) {
                expect(mockAnalysisService.analyzeProject).toHaveBeenCalledWith('proj-1', expect.anything());
            }
        });
    });
    ...
    describe('POST /projects/:projectId/shorts/:shortId/stems', () => {
        it('should trigger stem separation', async () => {
            mockStemService.separateStems.mockResolvedValue({ id: 'short-1', vocalsPath: '/tmp/vocals.wav' });

            const result = await controller.startStemSeparation?.('proj-1', 'short-1');

            if (result !== undefined) {
                expect(mockStemService.separateStems).toHaveBeenCalledWith('proj-1', 'short-1', expect.anything());
            }
        });
    });
});
